    ...
    <section class="content" style="margin-left:-1%;margin-right:-1%; margin-top:-1%;min-height:575px;">
     ...
     <div class="box">
      <div class="box-header with-border">
          <h3 class="box-title" style="font-weight:600">Edit/Delete Rescue Team Details</h3>
        </div>
          ...
          <div class="nav-tabs-custom responsive">
            <div class="tab-content">
              <div class="active tab-pane" id="activity">
                <!-- Post -->
				 <div class="box-body">
			<form class="form" name="frmsearch" method="post" action="list_rescue.php" style="width:100%;">
		...
		<form class="form-horizontal" name="frmnews" method="post" action="list_rescue.php" enctype="multipart/form-data">
		<div class="box box-primary">
          ...
          <td class="mailbox-name"><?php print $row['phone']; ?></td>
					...
					<a type="button" class="btn btn-default btn-sm" onclick="window.location='edit_rescue.php?rid=<?php print $row['rid']; ?>';"><i class="fa fa-pencil"></i></a>
					<a type="button" class="btn btn-default btn-sm" onclick="delete_temple(<?php print $row['rid']; ?>);"><i class="fa fa-trash"></i></a></td>
                   </tr>
				   <?php } ?></tbody>
                </table>
</div></div></div></form>
	...
	$(document).ready(function(){
		...
		{     
			...
			$(':checkbox:checked').each(function(i){
				rescue_id[i]=$(this).val();
			});
				if(rescue_id.length===0)
				{
				...
				else
				{
					$.ajax({
						url:'delete_rescue.php',
						method:'POST',
						data:{rescue_id:rescue_id},
						success:function()
						{
							for(var i=0;i<rescue_id.length;i++)
							{
								$('tr#'+rescue_id[i]+'').css('background-color', '#ccc');
								$('tr#'+rescue_id[i]+'').fadeOut('slow');
							}
						 window.location="list_rescue.php";
						}
